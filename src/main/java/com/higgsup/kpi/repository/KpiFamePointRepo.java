...
public interface KpiFamePointRepo extends CrudRepository<KpiFamePoint, Integer> {
    ...
    @Query(value = "SELECT * FROM kpi_fame_point where year = :year ORDER BY fame_point DESC, username ASC LIMIT :limitRows", nativeQuery = true)
    List<KpiFamePoint> getFamePointRanking(@Param("year") Integer year, @Param("limitRows") Integer limitRows);
}
