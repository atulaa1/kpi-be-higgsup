...
author: hiepnnt
purpose: add table kpi_personal_survey
...
CREATE TABLE `kpi_project_log` (
  ...
  FOREIGN KEY (`project_id`) REFERENCES `kpi_project` (`id`),
  FOREIGN KEY (`evaluation_id`) REFERENCES `kpi_evaluation` (`id`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
CREATE TABLE `kpi_personal_survey` (
  `id`                    INT(10) PRIMARY KEY AUTO_INCREMENT,
  `rated_username`        VARCHAR(50)
                          CHARACTER SET utf8 NOT NULL,
  ...
  `evaluation_id`         INT(11)         NOT NULL,
  FOREIGN KEY (evaluation_id) REFERENCES kpi_evaluation (`id`),
  FOREIGN KEY (question_id) REFERENCES kpi_survey_question_man(`id`),
  FOREIGN KEY (rated_username) REFERENCES kpi_user(user_name)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
CREATE TABLE `kpi_evaluation` (
  `id`                    INT(10) PRIMARY KEY AUTO_INCREMENT,
  `man_username`          VARCHAR(50)
                          CHARACTER SET utf8 NOT NULL,
  `evaluation_name`       VARCHAR(255)       NOT NULL,
  `created_date`          DATETIME           NOT NULL,
  `year_month_id`         INT(11)            NOT NULL,
  FOREIGN KEY (year_month_id) REFERENCES kpi_year_month (`id`),
  FOREIGN KEY (man_username) REFERENCES kpi_user(user_name)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;